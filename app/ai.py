...
extraction), narration (audio summaries), and quiz (question generation).
"""
import json
...
def extract_topics(text: str) -> list[str]:
    ...
    Returns a plain list of strings; empty list if AI isn't configured
...
def generate_narration_script(text: str, title: str) -> str:
    """
    Produces a spoken-style summary of a material for text-to-speech -
    explains the key ideas in plain language, includes an example where it
    helps understanding, and is written as natural spoken prose rather than
    a word-for-word reading of the source. Returns '' if generation fails.
    """
    if not client or not text.strip():
        return ""

    prompt = (
        f"You're preparing a short audio summary of a study material titled '{title}' "
        "for a university student to listen to. Write a clear, spoken-style summary of "
        "the key ideas - explain concepts in plain language, and include a simple example "
        "wherever it would genuinely help understanding. Do NOT read the material "
        "word-for-word; actually summarize and explain it, the way a tutor would talking "
        "it through out loud. Keep it focused - a few short paragraphs, not exhaustive. "
        "Write it as natural spoken prose with no markdown, no headers, no bullet points, "
        "since this will be converted directly to audio.\n\n"
        f"MATERIAL:\n{text[:12000]}"
    )
    try:
        response = client.models.generate_content(model=GEMINI_MODEL, contents=prompt)
